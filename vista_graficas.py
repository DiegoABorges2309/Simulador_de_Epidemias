...
class MiniGraficaCanvas(FigureCanvas):
    ...
    def __init__(self, titulo, color_linea, color_relleno):
        self.figura = Figure(figsize=(2.5, 1.4), facecolor='#111519')
    ...
    def redibujar_curva(self, x, y, indice_activo, dia_activo):
        ...
        self.ax.set_title(f"{self.titulo} | Val: {valor_actual:,}", 
                          color='#00f0ff', fontsize=8, family='monospace', loc='left', pad=4)
        
        self.ax.set_xlabel("Días", color='#687582', fontsize=7, family='monospace', labelpad=1)
        self.ax.set_ylabel("Cant.", color='#687582', fontsize=7, family='monospace', labelpad=1)
        ...
        self.ax.axvline(x=dia_activo, color='#00f0ff', linestyle='--', linewidth=1.2, zorder=5)
        self.draw_idle()
...
class GraficaCentralCanvas(FigureCanvas):
    ...
    def graficar_tendencia_maestra(self, dias, infectados_humanos):
        ...
        self.ax.set_title("CONTROL MAESTRO TEMPORAL (DESLIZA EL MOUSE SOBRE EL GRÁFICO)", 
                          color='#00f0ff', fontsize=9, fontweight='bold', pad=8)
        self.ax.set_xlabel("Eje de Tiempo Simulado (Días)", color='#a0aab5', fontsize=8, family='monospace')
        self.ax.set_ylabel("Infectados Humanos", color='#a0aab5', fontsize=8, family='monospace')
        
        for label in self.ax.get_xticklabels() + self.ax.get_yticklabels():
            label.set_family('monospace')
            
        self.linea_guia = self.ax.axvline(x=dias[0], color='#00f0ff', linewidth=2, alpha=0.8)
        self.draw()
...
class VistaGraficas(QWidget):
    ...
    def __init__(self):
        ...
        for canvas in self.mini_graficas_h.values():
            tarjeta = QWidget()
            tarjeta.setStyleSheet("background-color: #111519; border: 1px solid #1a232c; border-radius: 6px;")
        ...
        self.tarjeta_contador.setStyleSheet("background-color: #111519; border: 1px solid #00f0ff; border-radius: 6px;")
        ly_cont = QVBoxLayout(self.tarjeta_contador)
        lbl_c_tit = QLabel("DÍAS TRANSCURRIDOS")
        lbl_c_tit.setAlignment(Qt.AlignCenter)
        lbl_c_tit.setStyleSheet("color: #687582; font-size: 10px; font-weight: bold; letter-spacing: 1px;")
        self.lbl_dias_num = QLabel("0")
        self.lbl_dias_num.setAlignment(Qt.AlignCenter)
        self.lbl_dias_num.setStyleSheet("color: #00f0ff; font-family: 'Consolas'; font-size: 32px; font-weight: bold;")
        ...
        self.tarjeta_info.setStyleSheet("background-color: #111519; border: 1px solid #1a232c; border-radius: 6px;")
        ...
        for lbl in [self.lbl_pob_h, self.lbl_est_h, self.lbl_cen_v]:
            lbl.setStyleSheet("font-family: 'Consolas'; color: #cbd5e1; font-size: 11px; line-height: 16px;")
        ...
        self.canvas_central = GraficaCentralCanvas(callback_mouse=self.sincronizar_movimiento_interfaces)
        tarjeta_canvas = QWidget()
        tarjeta_canvas.setStyleSheet("background-color: #161a1e; border: 1px solid #1a232c; border-radius: 6px;")
    ...
    def sincronizar_movimiento_interfaces(self, dia_seleccionado, indice):
        ...
        sh, eh, ih, rh, mh = (self.datos_h["S"][indice], self.datos_h["E"][indice], 
                              self.datos_h["I"][indice], self.datos_h["R"][indice], self.datos_h["M"][indice])
        sv, ev, iv = (self.datos_v["S"][indice], self.datos_v["E"][indice], self.datos_v["I"][indice])

        # Actualizar textos del monitor central
        self.lbl_pob_h.setText(f"HUMANOS VIVOS\n» Susc: {int(sh):<5}\n» Recu: {int(rh):<5}")
        self.lbl_est_h.setText(f"CASOS ACTIVOS\n» Infec: {int(ih):<4}\n» Muert: {int(mh):<4}")
        self.lbl_cen_v.setText(f"POB. VECTORES\n» Susc: {int(sv):<5}\n» Infec: {int(iv):<5}")
        
        
        if ih < 1 and iv < 1:
            self.lbl_status.setText("ESTADO:\n[BROTE EXTINTO]")
            self.lbl_status.setStyleSheet("color: #34c759; font-weight: bold; font-size: 11px;")
        else:
            self.lbl_status.setText("ESTADO:\n[BROTE CRÍTICO]")
            self.lbl_status.setStyleSheet("color: #ff3b30; font-weight: bold; font-size: 11px;")

        # Redibujar sincrónicamente el cursor temporal en las 7 minigráficas
        for clave, canvas in self.mini_graficas_h.items():
            canvas.redibujar_curva(self.dias_x, self.datos_h[clave], indice, dia_seleccionado)
        for clave, canvas in self.mini_graficas_v.items():
            canvas.redibujar_curva(self.dias_x, self.datos_v[clave], indice, dia_seleccionado)
